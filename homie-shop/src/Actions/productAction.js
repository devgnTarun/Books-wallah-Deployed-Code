...
export const getProduct = (keyword = '' , currentPage = 1 , price=[0 , 40000] ,category) => async (dispatch) => {
    try {
        ...
        dispatch({
            type : EVERY_PRODUCT_SUCCESS,
            payload : data,
        })
    }
         catch (error) {
       dispatch({
        type : EVERY_PRODUCT_FAIL,
        payload : error.response.data.message
       }) 
    }
}

export const getCategoryProduct = (currentPage = 1 , price=[0 , 40000] ,category) => async (dispatch) => {
    try {
        
        dispatch({type : EVERY_PRODUCT_REQUEST});
       
        let link = `/api/v1/products?page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}`
        const {data} = await axios.get(link)
...
export const getProductDetails = (id) => async (dispatch) => {
    try {
        ...
        dispatch({type : PRODUCT_DETAIL__REQUEST});
